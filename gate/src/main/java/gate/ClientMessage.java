...
public class ClientMessage {
    ...
    public static void transfer2Logic(Message msg, ClientConnection conn) {
        ...
        if (msg instanceof Chat.CPrivateChat) {
            byteBuf = Utils
                    .pack2Server(msg, ParseMap.getPtoNum(msg), conn.getNetId(), Internal.Dest.Logic,
                            conn.getUserId());
        }
        GateMessageConnectionHandler.getGatelogicConnection().writeAndFlush(byteBuf);
    }
    ...
    public static void transfer2Auth(Message msg, ClientConnection conn) {
        ...
        if (msg instanceof Auth.CLogin) {
            ...
            byteBuf = Utils
                    .pack2Server(msg, ParseMap.getPtoNum(msg), conn.getNetId(), Internal.Dest.Auth,
                            userId);
